...
class SND:
    def __init__(self, energy=10000, two_theta=None, delay=0, SASE=False, ax=0, ay=0, cx=0, cy=0):

        if two_theta is not None:
            self.E0 = self.calc_energy(two_theta)
        else:
            self.E0 = energy
        self.cc_branch = Util.define_cc(self.E0,image_N=256)
        self.delay_branch = Util.define_delay(self.E0,delay=delay,image_N=256)
        ...
        self.b1 = beam.Beam(beam_params=self.beam_params)
        ...
        if SASE:
            self.pulse_delay = beam.Pulse(beam_params=self.beam_params, tau=10, time_window=50, SASE=True, num_spikes=10)
            self.pulse_cc = copy.deepcopy(self.pulse_delay)
        else:
            self.pulse_delay = beam.Pulse(beam_params=self.beam_params, tau=1, time_window=30)
            self.pulse_cc = beam.Pulse(beam_params=self.beam_params, tau=1, time_window=30)
...
class Util:
    ...
    def define_cc(E0, image_N=256):
        ...
        di = optics.PPM('di', z=1000, FOV=2e-3, N=image_N)
        ...
        cc1_1 = optics.Crystal('cc1_1', hkl=[2, 2, 0], E0=E0, z=t1_slit.z + 2. / 3., orientation=2, length=.02, width=.02,
                               show_figures=False)
        dci = optics.PPM('dci', z=cc1_1.z - 75e-3, N=image_N)
        gap = 55e-3
        ...
        cc1_2 = optics.Crystal('cc1_2', hkl=[2, 2, 0], E0=E0, z=cc1_1.z + dz, orientation=0, length=.06, width=.02,
                               show_figures=False)
        dcc = optics.PPM('dcc', z=t1_slit.z + 1, FOV=4e-3, N=image_N)
        ...
        cc2_1 = optics.Crystal('cc2_1', hkl=[2, 2, 0], E0=E0, z=cc2_2.z - dz, orientation=0, length=.06, width=.02,
                               show_figures=False)
        dco = optics.PPM('dco', z=cc2_2.z + 75e-3, FOV=4e-3, N=image_N)
        t4_slit = optics.Slit('t4_slit', y_width=4e-3, dy=2e-3, z=t1_slit.z + 2)
        do = optics.PPM('do', z=t4_slit.z + 0.15, FOV=2e-3, N=image_N)
        ...
        IP = optics.PPM('IP', FOV=50e-6, z=lens0.z + image, N=image_N)
        yag5 = optics.PPM('yag5', FOV=2e-3, z=IP.z - 1, N=image_N)

        device_list = [s4, di, t1_slit, dci, cc1_1, cc1_2, dcc, cc_shutter,
                       cc2_1, cc2_2, dco, t4_slit, do, lens0, lens1, yag5, IP]
    ...
    def define_bypass(E0):
        ...
        IP = optics.PPM('IP', FOV=50e-6, z=lens1.z + image)
    ...
    @staticmethod
    def define_delay(E0, delay=0, bypass=False, image_N=256):
        """
        Parameters
        ----------
        E0: photon energy (eV)
        ...
        t1_dh = optics.PPM('t1_dh', z=c1.z + dz2, FOV=4e-3, N=image_N)

        #     c2 = optics.Crystal('c2',hkl=[2,2,0],E0=E0,z=c1.z+dz,orientation=2,length=.02,width=.02)
        dd = optics.PPM('dd', z=c1.z + 1, FOV=2e-3, N=image_N)
        ...
        t4_dh = optics.PPM('t4_dh', z=c4.z - dz2, FOV=4e-3, N=image_N)

        t4_slit = optics.Slit('t4_slit', y_width=4e-3, dy=-2e-3, z=c4.z + 1e-6)

        do = optics.PPM('do', z=c4.z + 0.15, FOV=2e-3, N=image_N)
        lens0 = optics.CRL('lens0', E0=E0, f=3, z=do.z + 5, diameter=2e-3)
        lens1 = optics.CRL('lens1', E0=E0, f=3, z=do.z + 5 + 1e-6, orientation=1, diameter=2e-3)

        d_lens = optics.PPM('d_lens',z=lens0.z+.1, FOV=4e-3, N=image_N)
        image = 1 / (1 / 3.0 - 1 / (lens0.z - 624))

        # print(image)

        IP = optics.PPM('IP', FOV=50e-6, z=lens0.z + image,suppress=True, N=image_N)
        yag5 = optics.PPM('yag5', FOV=2e-3, z=IP.z-1,N=image_N)

        device_list = [s4, di, t1_slit, c1, t1_dh, c2, dd, delay_shutter,
                       c3, t4_dh, c4, t4_slit, do, lens0, lens1, d_lens, yag5, IP]
