...
class SND:
    ...
    def propagate_delay(self):
        self.delay_branch.propagate_beamline(self.b1)

    def propagate_cc(self):
        self.cc_branch.propagate_beamline(self.b1)
    ...
    def get_t1_dh_sum(self, noise_level=0):
        sig = self.delay_branch.t1_dh.profile.sum()
        sig += np.random.normal(scale=sig*noise_level)
        return sig

    def get_dd_sum(self,noise_level=0):
        sig = self.delay_branch.dd.profile.sum()
        sig += np.random.normal(scale=sig*noise_level)
        return sig

    def get_t4_dh_sum(self,noise_level=0):
        sig = self.delay_branch.t4_dh.profile.sum()
        sig += np.random.normal(scale=sig*noise_level)
        return sig

    def get_do_sum(self,noise_level=0):
        sig = self.delay_branch.do.profile.sum()
        sig += np.random.normal(scale=sig*noise_level)
        return sig

    def get_IP_sum(self,noise_level=0):
        sig = self.delay_branch.IP.profile.sum()
        sig += np.random.normal(scale=sig*noise_level)
        return sig
    ...
    def get_IP_cx(self,noise_level=0):
        cx = self.delay_branch.IP.cx + np.random.normal(scale=noise_level)
        return cx

    def get_IP_cy(self,noise_level=0):
        cy = self.delay_branch.IP.cy + np.random.normal(scale=noise_level)
        return cy

    def get_IP_r(self,noise_level=0):
        r = np.sqrt(self.get_IP_cx(noise_level)**2+self.get_IP_cy(noise_level)**2)
        return r
...
class Util:
    ...
    def define_cc(E0):
        ...
        device_list = [s3, di, t1_slit, dci, cc1_1, cc1_2, dcc, cc2_1, cc2_2, dco, t4_slit, do, lens0, lens1, d_lens, IP]
    ...
    def define_delay(E0, delay=0):
        s3 = optics.Slit('s3', z=990, x_width=.5e-3, y_width=1e-3)
        di = optics.PPM('di', z=1000, FOV=4e-3, N=256)
        ...
        IP = optics.PPM('IP', FOV=500e-6, z=lens1.z + image, blur=True, resolution=3e-6, N=512)
