...
class CurvedMirror(Mirror):
    ...
    def calc_misalignment(self, beam, cz):
        ...
        # plt.figure()
        # plt.plot(x1m,label='actual')
        # plt.plot(xIm,label='ideal')
        # plt.plot(height_error,label='residual')
...
class PPM:
    ...
    #
    #     # number of pixels to sum across to get lineout
    #     lineout_width = int(wfs.pitch / self.dx * 10 * mag)
    #
...
class CRL:
    ...
    def multiply(self, beam):
        ...
        if self.absorb:
            transmission = np.exp(-beam.k0 * beta * (thickness+self.web_thickness)) * np.exp(1j * phase) * mask# * np.exp(1j*phase_shift)
        else:
